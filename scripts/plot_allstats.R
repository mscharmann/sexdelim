...
colnames(mydata) <- c("chr","start","stop","log2_M_F_cov_ratio", "F_spec_kmers", "M_spec_kmers", "F_pi", "M_pi", "MvF_Fst", "MvF_dxy", "LD_r2", "GWAS_n_signif", "ZW_div", "XY_div", "het_m", "het_f")
mydata[mydata=="."] <- NA
...
bottom <- ggplot(data=mydata, aes(x=start, y=XY_div, group = chr)) +
	theme_classic() +
	geom_line(size = 0.1) +
...
pdf(args[2], width = 20, height = 22)
plot_grid(top, mid1, mid2, mid3, mid4, mid5, mid6, mid7, mid8, mid9, mid10, mid11, bottom, ncol=1, align = "v")
dev.off()
