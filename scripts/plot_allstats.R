...
colnames(mydata) <- c("chr","start","stop","log2_M_F_cov_ratio", "F_spec_kmers", "M_spec_kmers", "F_pi", "M_pi", "MvF_Fst", "MvF_dxy", "LD_r2", "GWAS_n_signif", "ZW_div", "XY_div")
mydata[mydata=="."] <- NA
...
mid9 <- ggplot(data=mydata, aes(x=start, y=ZW_div, group = chr)) +
	theme_classic() +
	geom_line(size = 0.1) +
	theme(axis.title.x = element_blank(),
	axis.text.x = element_blank()) +
